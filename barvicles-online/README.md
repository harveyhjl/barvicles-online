...
- Fixed 6/9 properly for online play:
  - it only starts if the next player actually has the required alternate card
  - if not, play resumes normally
  - no more being trapped by “next card must be 9”
- Changed top button text from **Restart game** to **New game**.
- Added short chat messages between players.
- Added fireworks when someone wins a set.
- Added a bigger Three Kings celebration when someone wins with 3 Kings.
