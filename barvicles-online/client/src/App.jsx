...
function playerColourClass(colour) {
  return `player-${colour || "green"}`;
}
...
function App() {
  ...
  function updateBethMode(enabled) {
    emit("updateFun", {
      roomCode,
      playerId,
      patch: {
  ...
  function updateMyColour(colour) {
    emit("updatePlayerColour", { roomCode, playerId, colour });
  }

  function sendReaction(text) {
    emit("sendReaction", { roomCode, playerId, text });
  }
      ...
      {state && (
        <>
          ...
          <div className="panel fun-panel">
            ...
            <label className="rule-toggle beth-mode">
              <input
                type="checkbox"
                checked={state.fun?.bethMode === true}
                onChange={e => updateBethMode(e.target.checked)}
              />
              <span>Beth Mode {state.fun?.bethMode ? "ON — Beth gets a cheeky edge" : "OFF"}</span>
            </label>

            <div className="colour-picker">
              <b>Your colour:</b>
              {(state.playerColours || ["green", "pink", "purple", "blue", "orange", "red"]).map(c => (
                <button
                  key={c}
                  className={`colour-dot ${c} ${state.you.colour === c ? "selected" : ""}`}
                  onClick={() => updateMyColour(c)}
                  title={c}
                />
              ))}
            </div>
            ...
            <div className="reaction-row">
              {(state.quickReactions || []).map(r => (
                <button className="reaction-button" key={r} onClick={() => sendReaction(r)}>{r}</button>
              ))}
            </div>
          </div>
          ...
          <div className="panel table multi-table">
            <div className="players-list">
              ...
              {(state.opponents || []).map(p => (
                <div className={`player-chip ${playerColourClass(p.colour)} ${p.isCurrentTurn ? "current" : ""}`} key={p.name}>
                  <b>{p.name} {p.isBot ? "🤖" : ""}</b>
            ...
            <div className="center">
              ...
              {state.lastAction?.message && (
                <p className="voice-line">{state.lastAction.playerName}: “{state.lastAction.message}”</p>
              )}
              <p>Pickup pile: {state.pickupPileCount}</p>
            </div>

            <div className={`you-panel ${playerColourClass(state.you.colour)}`}>
          ...
          {state.phase === "finished" && (state.lastAwards || []).length > 0 && (
            <div className="panel awards-panel">
              <h2>Round awards</h2>
              {(state.lastAwards || []).map((award, i) => (
                <div className="award" key={i}>🏆 {award}</div>
              ))}
            </div>
          )}
          ...
          <div className="panel chat-panel">
            ...
            <div className="chat-box">
              {(state.chat || []).map(msg => (
                <div className={`chat-message ${msg.type === "reaction" ? "reaction" : ""}`} key={msg.id}>
