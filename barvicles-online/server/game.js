...
const PLAYER_COLOURS = ["green", "pink", "purple", "blue", "orange", "red"];
const QUICK_REACTIONS = ["😂", "🙄", "👑", "NOPE.", "Jammy", "Robbed", "Skill issue", "Beth clears"];

const BOT_PERSONALITIES = [
  { name: "The Menace", lines: ["Calculated.", "Hold that.", "I fear nobody."] },
  { name: "The Idiot", lines: ["I have no clue.", "That was probably legal.", "Buttons are fun."] },
  { name: "The Coward", lines: ["Please don't hurt me.", "I regret everything.", "Mercy?"] },
  { name: "The Goblin", lines: ["Mine now.", "Chaos is breakfast.", "Hehehe."] }
];

function isBethPlayer(player) {
  return /beth/i.test(player?.name || "");
}

function defaultColourForIndex(i) {
  return PLAYER_COLOURS[i % PLAYER_COLOURS.length];
}

function cardVoiceLine(rank) {
  const lines = {
    A: "Suit switch. Nasty.",
    "2": "Pick up two. Rude.",
    "3": "Random robbery.",
    "4": "Four-card punishment.",
    "5": "King snatcher.",
    "6": "Six-nine chaos begins.",
    "7": "Sit down.",
    "8": "Have one of mine, unfortunately.",
    "9": "Six-nine chaos continues.",
    "10": "NOPE.",
    J: "Swap robbery.",
    Q: "Dump time.",
    K: "Royal behaviour."
  };
  return lines[rank] || "Card played.";
}

function initRoundStats(room) {
  room.roundStats = {};
  for (const p of room.players) {
    room.roundStats[p.id] = {
      plays: 0,
      nopes: 0,
      steals: 0,
      pickupsTaken: 0,
...
function incStat(room, playerId, key, amount = 1) {
  if (!room.roundStats) initRoundStats(room);
  if (!room.roundStats[playerId]) {
    room.roundStats[playerId] = { plays: 0, nopes: 0, steals: 0, pickupsTaken: 0, gifts: 0, sixNine: 0, queens: 0, kingsWin: 0 };
  }
  room.roundStats[playerId][key] = (room.roundStats[playerId][key] || 0) + amount;
}
...
function statLeader(room, key) {
  if (!room.roundStats) return null;
  let best = null;
  for (const [pid, stats] of Object.entries(room.roundStats)) {
...
function buildRoundAwards(room, winner, setWon, matchWon, reason) {
  const awards = [];
  awards.push(`Winner: ${winner.name}.`);
  if (String(reason || "").includes("3 Kings")) awards.push(`Royal moment: ${winner.name} ended it with 3 Kings.`);
  if (setWon) awards.push(`Set winner: ${winner.name}. Fireworks deserved.`);
  if (matchWon) awards.push(`Match winner: ${winner.name}. Brutal.`);

  const nope = statLeader(room, "nopes");
  if (nope) awards.push(`Most nope energy: ${playerNameById(room, nope.playerId)} (${nope.value}).`);

  const steal = statLeader(room, "steals");
  if (steal) awards.push(`Biggest thief: ${playerNameById(room, steal.playerId)}.`);

  const chaos = statLeader(room, "sixNine");
  if (chaos) awards.push(`Chaos gremlin: ${playerNameById(room, chaos.playerId)} kept 6/9 moving.`);

  const pickups = statLeader(room, "pickupsTaken");
  if (pickups) awards.push(`Most punished: ${playerNameById(room, pickups.playerId)} picked up ${pickups.value}.`);

  if (awards.length < 3) awards.push(`Most cursed card: ${top(room)?.rank || "unknown"}.`);
  return awards.slice(0, 5);
}

function applyBethModeEdge(room) {
  if (!room.fun?.bethMode) return;
  const beth = room.players.find(isBethPlayer);
  if (!beth) return;

  // Public, cheeky edge: Beth starts with one fewer card.
  if (beth.hand.length > 10) {
    const weakIdx = beth.hand.findIndex(c => !["10", "A", "2", "3", "4", "7", "J", "Q", "K"].includes(c.rank));
    const idx = weakIdx >= 0 ? weakIdx : beth.hand.length - 1;
    const [removed] = beth.hand.splice(idx, 1);
    room.deck.unshift(removed);
  }

  // If Beth has no nope, quietly swap a weak card for a 10 from the deck.
  if (!beth.hand.some(c => c.rank === "10")) {
    const deckTenIdx = room.deck.findIndex(c => c.rank === "10");
    const weakIdx = beth.hand.findIndex(c => !["A", "2", "3", "4", "7", "J", "Q", "K"].includes(c.rank));
    if (deckTenIdx >= 0 && weakIdx >= 0) {
      const [ten] = room.deck.splice(deckTenIdx, 1);
      const [swap] = beth.hand.splice(weakIdx, 1, ten);
      room.deck.unshift(swap);
    }
  }

  room.log.push(`Beth Mode is ON: ${beth.name} gets a cheeky competitive edge.`);
}

function pushChat(room, name, text, type = "chat") {
  if (!room.chat) room.chat = [];
  room.chat.push({ id: id(), name, text: String(text || "").slice(0, 120), type, at: Date.now() });
  room.chat = room.chat.slice(-30);
}

function maybeBotReaction(room, bot) {
  if (!bot?.isBot || Math.random() > 0.25) return;
  const personality = bot.personality || BOT_PERSONALITIES[0];
  const line = personality.lines[Math.floor(Math.random() * personality.lines.length)];
  pushChat(room, bot.name, line, "reaction");
}
...
function nextIndex(room, i, steps = 1) {
  if (!room.players.length) return 0;
  return (i + steps) % room.players.length;
}

// Backwards-compatible helper name.
// In 2-player this is the opponent; in 3/4-player this means the next player clockwise.
function otherIndex(room, i) {
  return nextIndex(room, i);
}
...
function finishRound(room, winnerIndex, reason) {
  ...
  room.lastAwards = buildRoundAwards(room, winner, setWon, matchWon, reason);
...
function pickupKingWithFive(room, player, cards) {
  ...
  room.lastAction = { type: "play", playerId: player.id, playerName: player.name, rank: "5", suit: realFive.suit, message: cardVoiceLine("5"), at: Date.now() };
  incStat(room, player.id, "plays");
  maybeBotReaction(room, player);
  return true;
}
...
export function createRoom(socketId, name) {
  ...
  rooms.set(code, {
    code,
    phase: "lobby",
    players: [{ id: playerId, socketId, name, hand: [], connected: true, saidBarvicles: false, isBot: false, colour: defaultColourForIndex(0) }],
    deck: [],
    discard: [],
    currentSuit: null,
    turn: 0,
...
export function joinRoom(code, socketId, name) {
  ...
  const playerId = id();
  room.players.push({ id: playerId, socketId, name, hand: [], connected: true, saidBarvicles: false, isBot: false, colour: defaultColourForIndex(room.players.length) });
  room.scores[playerId] = { rounds: 0, sets: 0 };
...
export function addComputerPlayer(code) {
  ...
  const personality = BOT_PERSONALITIES[(botNumber - 1) % BOT_PERSONALITIES.length];
  room.players.push({
    id: playerId,
    socketId: `bot-${code}`,
    name: `BarvBot ${botNumber}: ${personality.name}`,
    hand: [],
    connected: true,
    saidBarvicles: false,
    isBot: true,
    personality,
    colour: defaultColourForIndex(room.players.length)
  });
  room.scores[playerId] = { rounds: 0, sets: 0 };
  room.log.push(`${room.players[room.players.length - 1].name} joined.`);
  return playerId;
}
...
export function startGame(code) {
  const room = rooms.get(code);
  if (!room) throw new Error("Room not found");
  if (room.players.length < 2 || room.players.length > 4) throw new Error("Need 2 to 4 players");
...
export function playCards(code, playerId, cardIds, chosenSuit, saidBarvicles) {
  ...
  if (isThreeKingsWin && ruleOn(room, "threeKings")) {
    ...
    incStat(room, player.id, "kingsWin");
  ...
  if (room.queenDump?.active && room.queenDump.playerId === playerId) {
    ...
    room.lastAction = { type: "dump", playerId: player.id, playerName: player.name, rank: first.rank, suit: first.suit, message: "Dumped. No powers.", at: Date.now() };
    incStat(room, player.id, "plays");
    maybeBotReaction(room, player);
  ...
  room.log.push(`${player.name} played ${first.rank}${first.suit}. ${cardVoiceLine(first.rank)}`);
  room.lastAction = { type: "play", playerId: player.id, playerName: player.name, rank: first.rank, suit: first.suit, message: cardVoiceLine(first.rank), at: Date.now() };
  incStat(room, player.id, "plays");
  if (first.rank === "6" || first.rank === "9") incStat(room, player.id, "sixNine");
  maybeBotReaction(room, player);
  ...
  if (first.rank === "A" && ruleOn(room, "ace")) {
    ...
    incStat(room, player.id, "queens");
    specialApplied = true;
  }

  // 6/9 open-table chaos after a normal 6/9 play.
  if (first.rank === "6" || first.rank === "9") {
    clearNope(room);

    if (room.sixNine?.active) {
      updateSixNineAfterPlay(room, first.rank);
    } else {
  ...
  if (room.queenDump?.active) {
    ...
    const skippedIndex = nextIndex(room, i);
    room.turn = nextIndex(room, i, 2);
    room.log.push(`${room.players[skippedIndex].name} is skipped.`);
  } else {
    room.turn = nextIndex(room, i);
  }
}
...
export function drawCard(code, playerId) {
  ...
  if (room.pendingDraw > 0) {
    ...
    incStat(room, player.id, "pickupsTaken", room.pendingDraw);
    room.lastAction = { type: "draw", playerId: player.id, playerName: player.name, count: room.pendingDraw, message: `Picked up ${room.pendingDraw}.`, at: Date.now() };
    room.pendingDraw = 0;
    room.pendingRank = null;
    clearNope(room);
  } else {
    ...
    incStat(room, player.id, "pickupsTaken", 1);
    room.lastAction = { type: "draw", playerId: player.id, playerName: player.name, count: 1, message: "Picked up 1.", at: Date.now() };
  }
...
export function sendReaction(code, playerId, text) {
  const room = rooms.get(code);
  if (!room) throw new Error("Room not found");
  const i = playerIndex(room, playerId);
  const player = room.players[i];
  ...
  if (!clean) return;
  pushChat(room, player.name, clean, "reaction");
}

export function updateFun(code, playerId, patch) {
  const room = rooms.get(code);
  if (!room) throw new Error("Room not found");
  playerIndex(room, playerId);
  ...
  room.log.push(`Fun settings updated.`);
}

export function updatePlayerColour(code, playerId, colour) {
  const room = rooms.get(code);
  if (!room) throw new Error("Room not found");
  const i = playerIndex(room, playerId);
  const safe = PLAYER_COLOURS.includes(colour) ? colour : defaultColourForIndex(i);
  room.players[i].colour = safe;
  room.log.push(`${room.players[i].name} changed colour.`);
}
...
export function sendChat(code, playerId, text) {
  ...
  if (!clean) return;
  pushChat(room, player.name, clean, "chat");
}
...
export function getPublicState(code, playerId) {
  ...
  const opponents = room.players
    ...
    .map(p => ({
      ...
      isBot: !!p.isBot,
      colour: p.colour || defaultColourForIndex(room.players.indexOf(p)),
      isCurrentTurn: room.players[room.turn]?.id === p.id,
  ...
  return {
    ...
    you: {
      ...
      isBot: !!me.isBot,
      colour: me.colour || defaultColourForIndex(i),
    ...
    opponent: opp,
    opponents,
    currentPlayerName: room.players[room.turn]?.name || null,
    ...
    lastAwards: room.lastAwards || [],
    lastAction: room.lastAction || null,
    chat: room.chat || [],
    fun: room.fun || { bethMode: false },
