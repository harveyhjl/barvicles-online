...
function normalizeGameState(room) {
  ...
  if (room.sixNine?.active) {
    ...
    if (!ruleOn(room, "sixNine") || !playerHasRank(current, room.sixNine.nextRank)) {
      room.sixNine = null;
...
function updateSixNineAfterPlay(room, playedRank) {
  ...
  if (!playerHasRank(nextPlayer, nextRank)) {
    room.sixNine = null;
    room.turn = nextTurn;
    room.log.push(`6/9 chain ended. Play continues as normal.`);
    return;
...
function startSixNineIfNeeded(room, playedRank, playerIndexWhoPlayed) {
  ...
  // Online-safe version:
  // only continue if the next player actually has the required alternate card.
  // If not, the 6/9 rule ends and play resumes normally.
  if (!playerHasRank(nextPlayer, nextRank)) {
    room.sixNine = null;
    room.turn = nextTurn;
    room.log.push(`No ${nextRank} available for ${nextPlayer?.name || "next player"}. 6/9 chain ends.`);
    return false;
  }
