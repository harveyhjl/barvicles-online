...
function pickupKingWithFive(room, player, cards) {
  ...
  room.lastAction = { type: "play", playerId: player.id, playerName: player.name, rank: "5", suit: realFive.suit, at: Date.now() };
  return true;
}
...
export function addComputerPlayer(code) {
  ...
  room.players.push({
    id: playerId,
    socketId: `bot-${code}`,
    name: `BarvBot ${room.players.filter(p => p.isBot).length + 1}`,
    hand: [],
    connected: true,
    saidBarvicles: false,
    isBot: true
...
export function playCards(code, playerId, cardIds, chosenSuit, saidBarvicles) {
  ...
  if (room.queenDump?.active && room.queenDump.playerId === playerId) {
    ...
    room.lastAction = { type: "dump", playerId: player.id, playerName: player.name, rank: first.rank, suit: first.suit, at: Date.now() };
  ...
  room.lastAction = { type: "play", playerId: player.id, playerName: player.name, rank: first.rank, suit: first.suit, at: Date.now() };
...
export function drawCard(code, playerId) {
  ...
  if (room.pendingDraw > 0) {
    ...
    room.lastAction = { type: "draw", playerId: player.id, playerName: player.name, count: room.pendingDraw, at: Date.now() };
    room.pendingDraw = 0;
    room.pendingRank = null;
    clearNope(room);
  } else {
    ...
    room.lastAction = { type: "draw", playerId: player.id, playerName: player.name, count: 1, at: Date.now() };
  }
