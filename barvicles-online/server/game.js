...
function normalizeGameState(room) {
  ...
  // 6/9 safety: if the current player cannot play the required alternate,
  // end the chain instead of trapping the game.
  if (room.sixNine?.active) {
    const current = room.players[room.turn];
    if (!ruleOn(room, "sixNine") || !current || !playerHasRank(current, room.sixNine.nextRank)) {
      room.sixNine = null;
...
function updateSixNineAfterPlay(room, playedRank) {
  ...
  const nextTurn = otherIndex(room, room.turn);
  const nextPlayer = room.players[nextTurn];

  if (!nextPlayer || !playerHasRank(nextPlayer, nextRank)) {
    room.sixNine = null;
    room.turn = nextTurn;
    room.log.push(`6/9 chain ended. Play continues as normal.`);
    return;
...
function startSixNineIfNeeded(room, playedRank, playerIndexWhoPlayed) {
  ...
  const nextRank = rankNeededAfter(playedRank);
  const nextTurn = otherIndex(room, playerIndexWhoPlayed);
  const nextPlayer = room.players[nextTurn];

  // Practical online version: only start the chain if the next player
  // actually has the required alternate card. Otherwise normal play resumes.
  if (!nextPlayer || !playerHasRank(nextPlayer, nextRank)) {
    room.sixNine = null;
    room.turn = nextTurn;
    room.log.push(`No ${nextRank} available for the next player. 6/9 chain ends.`);
    return false;
  }
  ...
  room.sixNine = {
    active: true,
    nextRank,
    resumeTurn: playerIndexWhoPlayed
  };

  room.turn = nextTurn;
  room.log.push(`6/9 chain started. ${nextPlayer.name} must play ${nextRank}.`);
  return true;
}
