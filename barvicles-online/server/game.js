...
export function startGame(code) {
  ...
  while (["A", "2", "4", "7", "10", "J", "Q"].includes(top(room).rank)) {
    room.deck.unshift(room.discard.pop());
...
export function playCards(code, playerId, cardIds, chosenSuit, saidBarvicles) {
  ...
  // Q house rule: queen lets you dump up to 3 extra cards. First card must be Q.
  ...
  const played = cards[cards.length - 1];

  if (first.rank === "A") {
    if (!SUITS.includes(chosenSuit)) throw new Error("Ace needs a chosen suit");
    room.currentSuit = chosenSuit;
    room.log.push(`${player.name} played A and changed suit to ${chosenSuit}.`);
  } else {
    room.log.push(`${player.name} played ${cards.map(c => c.rank + c.suit).join(", ")}.`);
  }

  player.saidBarvicles = !!saidBarvicles;
  if (player.hand.length === 1 && !player.saidBarvicles) {
    const penalty = drawFromPile(room);
  ...
  // Card effects apply from the first special card played.
  let skipNext = false;
  ...
  if (skipNext) {
    room.turn = i; // player gets another go in two-player game
