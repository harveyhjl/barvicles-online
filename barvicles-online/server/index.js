...
io.on("connection", (socket) => {
  ...
  socket.on("sendReaction", ({ roomCode, playerId, text }, cb) => {
    try {
      sendReaction(roomCode, playerId, text);
      cb({ ok: true });
      emitRoom(roomCode);
    } catch (err) {
      cb({ ok: false, error: err.message });
  ...
  socket.on("updateFun", ({ roomCode, playerId, patch }, cb) => {
    try {
      updateFun(roomCode, playerId, patch);
      cb({ ok: true });
      emitRoom(roomCode);
    } catch (err) {
      cb({ ok: false, error: err.message });
  ...
  socket.on("updatePlayerColour", ({ roomCode, playerId, colour }, cb) => {
    try {
      updatePlayerColour(roomCode, playerId, colour);
      cb({ ok: true });
      emitRoom(roomCode);
    } catch (err) {
      cb({ ok: false, error: err.message });
