...
const SPECIALS_NOPEABLE = ["A", "2", "3", "4", "7", "8", "J", "Q"];
...
function canNopeNow(card, room, playerId) {
  if (card.rank !== "10") return false;

  // 10 can respond to a pending pickup.
  if (room.pendingDraw > 0) return true;

  // 10 can nope a previous nope.
  if (top(room)?.rank === "10" && room.nopedEffect) return true;

  // 10 can nope the last nopeable special effect.
  if (room.lastNopeableEffect && room.lastNopeableEffect.sourcePlayerId !== playerId) return true;
...
function canPlayOn(card, room, playerId) {
  ...
  // 10 can nope almost every special effect.
  if (canNopeNow(card, room, playerId)) return true;
...
function resetHandsAndDeck(room) {
  ...
  // Winner takes first go in the next game.
  ...
  room.nopedDraw = 0;
  room.nopedRank = null;
  room.lastNopeableEffect = null;
  room.nopedEffect = null;
  room.winner = null;
}
...
function finishGame(room, winnerIndex, reason) {
  ...
  room.log.push(`${winner.name} starts the next game.`);
}

function finishIfAnyPlayerHasNoCards(room, fallbackWinnerIndex = 0) {
  const winnerIndex = room.players.findIndex(p => p.hand.length === 0);
  if (winnerIndex !== -1) {
    clearNopeMemory(room);
    finishGame(room, winnerIndex, `${room.players[winnerIndex].name} wins with no cards left.`);
    return true;
  }
  return false;
}

function clearNopeMemory(room) {
  room.lastNopeableEffect = null;
  room.nopedEffect = null;
}

function recordNopeableEffect(room, effect) {
  room.lastNopeableEffect = effect;
  room.nopedEffect = null;
}

function undoEffect(room, effect) {
  if (!effect) return;

  if (effect.type === "A") {
    room.currentSuit = effect.previousSuit;
    room.log.push(`Change suit was noped. Suit goes back to ${effect.previousSuit}.`);
  }

  if (effect.type === "2" || effect.type === "4") {
    room.pendingDraw = effect.previousPendingDraw || 0;
    room.pendingRank = effect.previousPendingRank || null;
    room.log.push(`Pickup chain was noped.`);
  }

  if (effect.type === "3" || effect.type === "8") {
    const fromPlayer = room.players.find(p => p.id === effect.fromPlayerId);
    const toPlayer = room.players.find(p => p.id === effect.toPlayerId);
    if (fromPlayer && toPlayer) {
      const idx = findCardIndex(toPlayer.hand, effect.card.id);
      if (idx !== -1) {
        const [card] = toPlayer.hand.splice(idx, 1);
        fromPlayer.hand.push(card);
        room.log.push(`Random card transfer was noped.`);
      }
    }
  }

  if (effect.type === "J") {
    const a = room.players.findIndex(p => p.id === effect.playerAId);
    const b = room.players.findIndex(p => p.id === effect.playerBId);
    if (a !== -1 && b !== -1) {
      [room.players[a].hand, room.players[b].hand] = [room.players[b].hand, room.players[a].hand];
      room.log.push(`Hand swap was noped. Hands swapped back.`);
    }
  }

  if (effect.type === "7") {
    const idx = room.players.findIndex(p => p.id === effect.skippedPlayerId);
    if (idx !== -1) {
      room.turn = idx;
      room.log.push(`Skip was noped. ${room.players[idx].name} gets their turn.`);
    }
  }

  if (effect.type === "Q") {
    const queenPlayer = room.players.find(p => p.id === effect.sourcePlayerId);
    if (queenPlayer) {
      // Return the dumped cards, but not the Queen itself.
      for (const dumpedCard of effect.dumpedCards) {
        const discardIdx = findCardIndex(room.discard, dumpedCard.id);
        if (discardIdx !== -1) {
          const [card] = room.discard.splice(discardIdx, 1);
          queenPlayer.hand.push(card);
        }
      }
      const newTop = top(room);
      if (newTop) room.currentSuit = newTop.suit;
      room.log.push(`Queen dump was noped. Dumped cards returned.`);
    }
  }
}
...
export function playCards(code, playerId, cardIds, chosenSuit, saidBarvicles) {
  ...
  const isNopeMove = cards.length === 1 && first.rank === "10" && canNopeNow(first, room, playerId);

  // Normal moves require turn. 6/9 chaos and nope responses can interrupt.
  if (i !== room.turn && !isChaosMove && !isNopeMove) throw new Error("Not your turn");
  ...
  if (!canPlayOn(first, room, playerId)) throw new Error("First card is not playable");
  ...
  const previousSuit = room.currentSuit;
  const previousPendingDraw = room.pendingDraw;
  const previousPendingRank = room.pendingRank;
  ...
  if (first.rank === "10" && isNopeMove) {
    // Nope a nope: bring back the previously noped effect.
    if (room.nopedEffect && top(room).rank === "10") {
      const restored = room.nopedEffect;

      if (restored.type === "2" || restored.type === "4") {
        room.pendingDraw = restored.afterPendingDraw || 0;
        room.pendingRank = restored.afterPendingRank || null;
        room.log.push(`${player.name} noped the nope. Pickup chain is back.`);
      } else {
        // For non-pickup effects, a nope-a-nope means the effect stays as originally played.
        room.log.push(`${player.name} noped the nope. Original effect stands.`);
      }

      room.lastNopeableEffect = restored;
      room.nopedEffect = null;
    } else if (room.lastNopeableEffect) {
      const cancelled = room.lastNopeableEffect;
      undoEffect(room, cancelled);
      room.nopedEffect = cancelled;
      room.lastNopeableEffect = null;
      room.log.push(`${player.name} played 10 and noped ${cancelled.type}.`);
    } else if (room.pendingDraw > 0) {
      room.nopedEffect = {
        type: room.pendingRank || "pickup",
        afterPendingDraw: room.pendingDraw,
        afterPendingRank: room.pendingRank,
        sourcePlayerId: room.players[otherIndex(room, i)]?.id
      };
      room.pendingDraw = 0;
      room.pendingRank = null;
    ...
    recordNopeableEffect(room, {
      type: "7",
      sourcePlayerId: player.id,
      skippedPlayerId: room.players[otherIndex(room, i)].id
    });
    ...
    recordNopeableEffect(room, {
      type: "J",
      sourcePlayerId: player.id,
      playerAId: room.players[i].id,
      playerBId: room.players[oi].id
    });
    ...
    const dumpedCards = cards.slice(1);
    if (dumpedCards.length > 0) {
      recordNopeableEffect(room, {
        type: "Q",
        sourcePlayerId: player.id,
        dumpedCards
    ...
    if (room.pendingDraw === 0) room.pendingRank = null;
    if (!isChaosMove) clearNopeMemory(room);
  }

  // A player can reach zero cards because of their own play OR because another player
  // used a card like 3 to take their last card. Whoever has zero cards wins.
  if (!jackSwapped && finishIfAnyPlayerHasNoCards(room, i)) {
    return;
  }
